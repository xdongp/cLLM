...
#### 3.1.1 CPU 端进一步优化点（建议补充实现）

**SIMD 与微内核优化**
- **对齐与布局**：确保 `head_dim` / `embedding_dim` 的内存对齐（32/64 字节），避免跨 cache line 的加载，提升 SIMD 吞吐。
- **向量化友好布局**：对 GEMM 输入采用 `K` 维连续布局，减少 stride；对 KV cache 保持连续写入路径。
- **混合精度策略**：Embedding/LMHead 使用 F16，FFN/Attention 使用 Q4_K_M 或 Q5_K_M，以减少带宽压力。

**算法级优化**
- **Prefill 与 Decode 分离**：预填充阶段一次构图、批量算子融合；解码阶段使用轻量图或单 token 计算路径，减少构图开销。
- **注意力优化**：短序列走标准 softmax；长序列启用 `ggml_flash_attn_ext`，避免中间 `QK^T` 全量物化。
- **FFN 结构优化**：对 SwiGLU 中 `gate`/`up` 的并行 GEMM 进行融合，减少一次中间张量写回。

**线程与调度优化**
- **线程数策略**：默认取物理核数；当模型较小或 batch size 很小，限制线程数避免调度开销压过计算收益。
- **线程亲和**：在 NUMA/多插槽环境建议绑定线程与内存（如 `numactl` 或平台绑定策略）。
- **分块尺寸调优**：基于 `L2/L3` 大小选择 block size，减少缓存抖动。

**内存与缓存优化**
- **KV Cache 分块/分页**：按固定 block 大小（如 256/512 token）管理 KV，减少缓存 miss 与碎片化。
- **重复张量复用**：构图阶段复用临时张量，降低分配次数与内存峰值。
- **预取与流水**：关键路径可显式预取下一层权重或 KV block，提升流水线效率。
