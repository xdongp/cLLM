...
#### 3.2.1 LlamaCppTokenizer 实现

```cpp
class LlamaCppTokenizer : public ITokenizer {
private:
    // llama.cpp的词汇表结构指针
    struct llama_vocab* vocab_;
...
private:
    ...
    void applyPreprocessing(const std::string& modelType);
};
```
...
private:
    ...
    std::string applyDeepSeekLLMPreprocessing(const std::string& text) {
        // DeepSeek LLM使用的预处理逻辑
        // 使用llama.cpp内置的LLAMA_VOCAB_PRE_TYPE_DEEPSEEK_LLM预处理器
        // 正则表达式模式：
        // - "[\r\n]": 匹配换行符
        // - "\\s?[A-Za-z...]": 匹配字母字符
        // - "\\s?[!-/:-~...]": 匹配标点符号
        // - "[一-龥...]": 匹配中文字符
        // - "\\p{N}+": 匹配数字
        return text; // 实际实现将通过llama.cpp的预处理器处理
    }
    
    std::string applyDeepSeekCoderPreprocessing(const std::string& text) {
        // DeepSeek Coder使用的预处理逻辑
        // 使用llama.cpp内置的LLAMA_VOCAB_PRE_TYPE_DEEPSEEK_CODER预处理器
        // 正则表达式模式：
        // - "[\r\n]": 匹配换行符
        // - "\\s?\\p{L}+": 匹配字母
        // - "\\s?\\p{P}+": 匹配标点
...
private:
    ...
    std::string applyQwenPreprocessing(const std::string& text) {
        // Qwen2使用的预处理逻辑
        // 使用llama.cpp内置的LLAMA_VOCAB_PRE_TYPE_QWEN2预处理器
        // 正则表达式模式：
...
| GGUF | model.gguf | llama.cpp加载 | llama.cpp模型 |
...
### 5.1 llama.cpp 集成方案
- 使用llama.cpp的`llama_vocab`结构进行底层分词
- 封装llama.cpp的分词算法实现
- 保持与现有API的兼容性
- 实现llama.cpp预处理器类型的支持

### 5.2 逐步迁移策略
1. **第一阶段**: 实现基础llama.cpp分词器封装
...
### 9.1 llama.cpp 集成要点
- 需要正确初始化llama.cpp的vocab结构
- 不同预处理器类型对应不同的正则表达式模式
- 注意内存管理，避免内存泄漏
- 预处理器类型包括：LLAMA_VOCAB_PRE_TYPE_DEFAULT、LLAMA_VOCAB_PRE_TYPE_LLAMA3、LLAMA_VOCAB_PRE_TYPE_DEEPSEEK_LLM、LLAMA_VOCAB_PRE_TYPE_DEEPSEEK_CODER、LLAMA_VOCAB_PRE_TYPE_QWEN2等
