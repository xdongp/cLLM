...
**阶段 1: 初始探测阶段（探索）**
- **目标**: 快速找到可行的 batch size 范围
- **策略**: 二分查找 + 指数增长
- **步骤**:
  1. 从最小 batch size (1) 开始
  2. 指数增长: 1 → 2 → 4 → 8 → 16 → ...
  3. 直到出现内存溢出或性能下降
  4. 使用二分查找精确定位最优值

**阶段 2: 动态调整阶段（优化）**
- **目标**: 在运行时持续优化 batch size
- **策略**: 爬山算法 + 动量调整
- **调整规则**:
```
if (当前性能 > 历史最佳 * (1 + ε)) {
    // 性能提升，继续增大 batch size
    batch_size = min(batch_size * (1 + α), max_batch_size)
} else if (当前性能 < 历史最佳 * (1 - ε)) {
    // 性能下降，减小 batch size
    batch_size = max(batch_size * (1 - β), min_batch_size)
} else {
    // 性能稳定，小幅震荡探索
    batch_size = batch_size * (1 ± δ)
}
...
- **收敛条件**:
```
连续 N 个周期内，batch size 变化 < θ
且性能波动 < φ

其中:
- N: 稳定周期数（如 10）
- θ: batch size 变化阈值（如 10%）
- φ: 性能波动阈值（如 3%）
```
...
- P: 并行化比例
- N: 并行度（batch size）
- 推论: 增大 batch size 可以提高并行度，从而加速
```

**2. Roofline 模型**
```
Performance = min(Compute_Bound, Memory_Bound)

- Compute_Bound: 受限于 GPU FLOPS
- Memory_Bound: 受限于内存带宽
- 最优 batch size 应使两者达到平衡
```

**3. 收益递减定律**
```
当 batch size 超过某个阈值后:
- 内存占用急剧增加
- 可能导致内存溢出
- 反而降低吞吐量
- 需要找到收益最大的平衡点
...
**文件 1: `src/scheduler/dynamic_batch_tuner.h`**
```cpp
/**
 * @file dynamic_batch_tuner.h
 * @brief 动态 Batch Size 调谐器
 * 
 * 负责:
 * - GPU 性能探测
 * - Batch size 计算与调整
...
private:
    ...
    std::atomic<size_t> currentMinBatchSize_;
...
void Scheduler::processRequests() {
    ...
    size_t minBatchSize = batchTuner_->getOptimalBatchSize();
    constexpr size_t MAX_WAIT_MS_FOR_BATCH = 50;
...
dynamic_batch_tuner:
  ...
  # 调整阶段参数
  performance_threshold: 0.05      # 性能变化阈值 (ε)
  growth_coefficient: 0.2          # 增长系数 (α)
  decay_coefficient: 0.3           # 衰减系数 (β)
  oscillation_coefficient: 0.05    # 震荡系数 (δ)
  
  # 稳定阶段参数
  stabilization_cycles: 10         # 稳定周期数 (N)
  batch_size_change_threshold: 0.10  # batch size 变化阈值 (θ)
  performance_fluctuation_threshold: 0.03  # 性能波动阈值 (φ)
  
  # 安全参数
  memory_safety_margin_mb: 512     # 内存安全余量
  memory_usage_limit: 0.90         # 内存使用限制 (90%)
  max_consecutive_failures: 3      # 最大连续失败次数
  
  # 性能权重
  throughput_weight: 0.5           # 吞吐量权重 (α)
  gpu_utilization_weight: 0.3      # GPU 利用率权重 (β)
  memory_efficiency_weight: 0.2    # 内存效率权重 (γ)
```
