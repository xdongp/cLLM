...
MODEL_PATH ?= $(HOME)/model/Qwen/Qwen3-0.6B
PORT ?= 8080
QUANTIZATION ?= fp16
MAX_BATCH_SIZE ?= 8
...
help:
	...
	@echo "  MODEL_PATH           - Path to model directory (default: ~/model/Qwen/Qwen3-0.6B)"
	...
	@echo "  make start           - Start server with default model"
	@echo "  make start MODEL_PATH=/path/to/model PORT=9000"
	@echo "  make stop            - Stop the server"
	@echo ""
	@echo "Note:"
	@echo "  - http_server_example and basic_usage examples are deprecated"
	@echo "  - Use 'make start' to run the production cLLM server"
	@echo "  - Main server executable (cllm_server) is not yet implemented"
	@echo "  - Currently only unit tests are functional"
