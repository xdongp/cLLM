...
std::vector<llama_token> SentencePieceTokenizer::encode(const std::string& text, bool addSpecialTokens) {
    ...
    if (cacheEnabled_) {
        if (auto cached = tokenCache_.getEncode(text)) {
            // 将 int 向量转换为 llama_token
            std::vector<llama_token> cachedTokens;
            cachedTokens.reserve(cached->size());
            for (int id : *cached) {
                cachedTokens.push_back(static_cast<llama_token>(id));
            }
            return cachedTokens;
        }
    }
    ...
    // 写入缓存
    if (cacheEnabled_ && !ids.empty()) {
        tokenCache_.putEncode(text, ids);
    }
...
std::string SentencePieceTokenizer::decode(const std::vector<llama_token>& ids, bool skipSpecialTokens) {
    ...
    // 将 llama_token 转换为 int 以便缓存 key 使用
    ...
    if (cacheEnabled_) {
        if (auto cached = tokenCache_.getDecode(int_ids)) {
            return *cached;
        }
    }
    ...
    if (cacheEnabled_ && !int_ids.empty() && !result.empty()) {
        tokenCache_.putDecode(int_ids, result);
    }
