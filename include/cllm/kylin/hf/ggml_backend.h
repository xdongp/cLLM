...
private:
    ...
    ggml_tensor* graphLayer1Output_ = nullptr;   // Layer 1 最终输出 (用于调试)
    ggml_tensor* graphAttnOutput_ = nullptr;     // Attention 输出 (用于调试)
    ggml_tensor* graphAttnInput_ = nullptr;      // Attention 输入 (用于调试)
    ggml_tensor* graphFFNInput_ = nullptr;       // FFN 输入 (用于调试)
    ggml_tensor* graphFFNGate_ = nullptr;        // FFN Gate 输出 (用于调试)
    ggml_tensor* graphFFNUp_ = nullptr;          // FFN Up 输出 (用于调试)
    ggml_tensor* graphFFNDown_ = nullptr;        // FFN Down 输出 (用于调试)
    ggml_tensor* graphPostNorm_ = nullptr;       // Post Attention RMS Norm 输出 (用于调试)
    ggml_tensor* graphPostNormRaw_ = nullptr;    // Post Attention RMS Norm 原始输出 (用于调试)
